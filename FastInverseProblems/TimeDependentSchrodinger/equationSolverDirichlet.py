...
for k in ks:
    ...
    finalX = 5
    numX = 500
    domain = mesh.create_interval(MPI.COMM_WORLD, numX, [initialX, finalX])
    ...
    x0    = -2.0                        # center of initial gaussian wave-packet
    ...
    '''
    a = (inner(psi, u)+ 1j * dt/2 * (0.5 * inner(grad(psi), grad(u)) + inner(V_pot * psi, u))) * dx
    L = (inner(psi_n, u)- 1j * dt/2 * (0.5 * inner(grad(psi_n), grad(u)) + inner(V_pot * psi_n, u))) * dx
    ...
    if MPI.COMM_WORLD.rank == 0:
        ...
        print((np.abs(psi_sol.x.array)**2).shape)
        #dof_coords = V.tabulate_dof_coordinates().reshape((-1, domain.geometry.dim))
        dof_coords = V.tabulate_dof_coordinates()[:, 0]  # Extract the x-coordinate (since you're 1D)
        ...
        plt.close()
        #print("probability densities plotted")
    ...
    random.seed(43)
    def getRandomPoints(numpoints,data,minx, maxx,numx,mint,maxt,numt):
        points = np.zeros((numpoints,3),dtype=complex)
        for i in range(numpoints):
            randxindex = random.randint(0,numx - 1)
            randtindex = random.randint(0,numt - 1)
            points[i,2] = data[randtindex,randxindex]
            points[i,0] = mint + (maxt - mint)*(randtindex/(numt - 1))
            points[i,1] = minx + (maxx - minx)*(randxindex/(numx - 1))
        return points
    
    randomPoints = getRandomPoints(10,psi_data,initialX,finalX,numX,0,T,num_steps)
    np.save(f"data_10DPts/dataKis{k}.npy",randomPoints)
